...
def get_demo_events():
    return [
        [1, "visitor_entered", "V001", "CAM01", "2026-06-02 10:00"],
        [2, "visitor_entered", "V002", "CAM01", "2026-06-02 10:05"],
        [3, "visitor_entered", "V003", "CAM02", "2026-06-02 10:10"],
        [4, "visitor_entered", "V004", "CAM02", "2026-06-02 10:15"],
        [5, "visitor_entered", "V005", "CAM03", "2026-06-02 10:20"],
        [6, "visitor_entered", "V006", "CAM04", "2026-06-02 10:25"],
        [7, "visitor_entered", "V007", "CAM05", "2026-06-02 10:30"],
        [8, "visitor_entered", "V008", "CAM05", "2026-06-02 10:35"],
        [9, "visitor_entered", "V009", "CAM05", "2026-06-02 10:40"],
        [10, "visitor_entered", "V010", "CAM05", "2026-06-02 10:45"],
        [11, "visitor_entered", "V011", "CAM05", "2026-06-02 10:50"],
        [12, "visitor_entered", "V012", "CAM05", "2026-06-02 10:55"],
        [13, "visitor_entered", "V013", "CAM05", "2026-06-02 11:00"],
    ]


def fetch_events():
    try:
        response = requests.get(f"{API}/events", timeout=3)
        response.raise_for_status()
        data = response.json()
        if data:
            return data, "live"
        return get_demo_events(), "demo"
    except Exception:
        return get_demo_events(), "demo"
...
col_status1.markdown('<div class="status-box"><span class="glow-green"></span>FastAPI Online</div>', unsafe_allow_html=True)
col_status2.markdown('<div class="status-box"><span class="glow-green"></span>SQLite Connected</div>', unsafe_allow_html=True)
col_status3.markdown('<div class="status-box"><span class="glow-purple"></span>YOLOv8 Active</div>', unsafe_allow_html=True)
...
video_options = {
    "CAM 1 - Store Zone": {"video": "data/CAM 1.mp4", "yolo": "data/yolo_CAM1.mp4", "id": "CAM01", "zone": "Store Zone"},
    "CAM 2 - Product Zone": {"video": "data/CAM 2.mp4", "yolo": "data/yolo_CAM2.mp4", "id": "CAM02", "zone": "Product Zone"},
    "CAM 3 - Entry/Exit": {"video": "data/CAM 3.mp4", "yolo": "data/yolo_CAM3.mp4", "id": "CAM03", "zone": "Entry / Exit"},
    "CAM 4 - Staff/Backroom": {"video": "data/CAM 4.mp4", "yolo": "data/yolo_CAM4.mp4", "id": "CAM04", "zone": "Staff / Backroom"},
    "CAM 5 - Billing Area": {"video": "data/CAM 5.mp4", "yolo": "data/yolo_CAM5.mp4", "id": "CAM05", "zone": "Billing Area"},
}
...
data, data_mode = fetch_events()

if data_mode == "demo":
    st.info("Demo mode active: backend API not reachable, showing sample retail intelligence data.")
...
if len(data) > 0:
    ...
    ex1.markdown(f'<div class="kpi-card"><div class="kpi-label">Visitors Today</div><div class="kpi-value">{entered_today}</div></div>', unsafe_allow_html=True)
    ex2.markdown(f'<div class="kpi-card"><div class="kpi-label">Current Occupancy</div><div class="kpi-value">{current_occupancy}</div></div>', unsafe_allow_html=True)
    ex3.markdown(f'<div class="kpi-card"><div class="kpi-label">Peak Hour</div><div class="kpi-value" style="font-size:26px;">{peak_hour}</div></div>', unsafe_allow_html=True)
    ex4.markdown(f'<div class="kpi-card"><div class="kpi-label">Crowd Alerts</div><div class="kpi-value">{crowd_alerts}</div></div>', unsafe_allow_html=True)
    ex5.markdown(f'<div class="kpi-card"><div class="kpi-label">Health Score</div><div class="kpi-value">{store_health_score}%</div></div>', unsafe_allow_html=True)
    ...
    with col_video1:
        ...
        if os.path.exists(original_path):
            ...
            st.warning("Original CCTV video not available in deployed demo.")
    ...
    with col_video2:
        ...
        if os.path.exists(yolo_path):
            ...
            st.warning("YOLO output video not available in deployed demo.")
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="ai-box">{recommendation}</div>', unsafe_allow_html=True)
    ...
    b1.markdown(f'<div class="business-box"><b>Most Active Camera</b><br><br>{most_active}</div>', unsafe_allow_html=True)
    b2.markdown(f'<div class="business-box"><b>Conversion Opportunity</b><br><br>{conversion_rate}%</div>', unsafe_allow_html=True)
    b3.markdown(f'<div class="business-box"><b>Occupancy</b><br><br>{current_occupancy} customers</div>', unsafe_allow_html=True)
    b4.markdown(
    ...
    st.warning("No events found. Demo data could not be loaded.")
